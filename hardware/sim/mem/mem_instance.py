...
# Manual configuration for 16nm node (scaled from 22nm)
# Scaling factors based on process scaling theory; adjust with real data if needed
MANUAL_16NM_CONFIG = {
...
class MemoryInstance:
    ...
    def __init__(
        ...
        if get_cost_from_cacti:
            ...
            # Special handling for 16nm node (not natively supported by CACTI)
            if mem_config['technology'] == 0.016:
                # Use 22nm CACTI results and apply scaling factors
                ...
                # Update mem_config for consistency
                mem_config.update({
                    'r_cost': self.r_cost,
                    'w_cost': self.w_cost,
                    'area': self.area,
                ...
                # Other nodes use CACTI directly
